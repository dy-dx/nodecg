...
app.get('/view/:bundle_name*', function(req, res) {
    ...
    var fileLocation = path.join(__dirname, '../../bundles/', bundleName, 'view/', resName);

    // Check if the file exists
    if (!fs.existsSync(fileLocation)) {
        res.status(404).send('Unable to find '+ bundleName +'/'+ resName);
    }

    res.sendFile(fileLocation);
});

app.get('/admin/:bundle_name*', function(req, res) {
    var bundleName = req.param('bundle_name');
    ...
    var fileLocation = path.join(__dirname, '../../bundles/', bundleName, 'admin/', resName);
